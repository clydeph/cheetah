...
int HDF5_ReadHeader(const char *filename, h5_info_t *result) {
	...
	if (dataspace < 0)
	{
		printf("ERROR: Could not get dataspace %s\n", h5field);
		return -1;
	}
	...
	if (rank < 0)
	{
		printf("ERROR: Could not get rank of dataspace %s\n", h5field);
		return -1;
	}
	...
	if (status_n < 0)
	{
		printf("ERROR: Could not get dimensions %s\n", h5field);
		return -1;
	}
	...
	for(uint32_t i=0; i < result->ndetectors; i++)
	{
		...
		printf("Detector_%d Data Shape: RANK %d, (%d", i, rank, (int)(dims_out[0]));
		for(int j=1; j<rank; j++) printf(" x %d", (int)dims_out[j]);
		printf(")\n");
...
int HDF5_cleanup(h5_info_t *header) {
	...
	for(uint32_t i = 0; i < header->ndetectors; i++)
	{
