...
def extract_template(self):
    ...
    # PAL-XFEL
    if realdir.startswith("/xfel/l"):
        print("Extracting template for PAL-XFEL...")
        extract_pal_template(self)
        return
...
#
#   Extract the PAL template and modify to suit determined experiment
#
def extract_pal_template(self):
    #   Deduce experiment number, etc using de-referenced paths
    #   Assumes the file path follows the pattern:   /reg/d/psdm/cxi/cxij4915/scratch/...
    realdir = os.getcwd()

    #   Now for some PAL-specific stuff
    #   /xfel/ffs/dat/ue_191103_SFX/raw_data/123456/...
    ss = realdir.split('/')
    ss = ss[1:]

    # ss = ['xfel', 'ffs', 'dat', 'ue_191103_SFX']
    instr = 'cxi'
    expt = ss[3]
    datadir = '/' + str.join('/', ss[0:4]) + '/raw_data' # '/xfel/ffs/dat/ue_191027_SFX/raw_data'
    userdir = '/' + str.join('/', ss[0:4]) + '/proc/cheetah' # '/xfel/ffs/dat/ue_191027_SFX/proc/cheetah'
    ...
    # (Detector name, encoders, etc)
    #
    modify_PAL_cheetah_config_files(self)

    return
    #end extract_pal_template()
...
#   Called as a separate function so that it can be called from the menu as well as on startup
#
def modify_PAL_cheetah_config_files(self):

    # Dialog box
    result, ok = gui_dialogs.configure_cheetah_pal_gui.configure_cheetah_dialog()
